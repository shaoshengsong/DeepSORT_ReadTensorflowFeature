...
void postprocess(Mat& frame, const vector<Mat>& out,   DETECTIONS& d);
  ...
  while (waitKey(1) < 0)
    {
      ...
      if(FeatureTensor::getInstance()->getRectsFeature(frame, detections) == false) {
          cout << "Tensorflow get feature failed!"<<std::endl;;
        }
      else
        {
          cout << "Tensorflow get feature succeed!"<<std::endl;;
        }
...
// Remove the bounding boxes with low confidence using non-maxima suppression
void postprocess(Mat& frame, const vector<Mat>& outs,DETECTIONS& d)
{
  vector<int> classIds;
